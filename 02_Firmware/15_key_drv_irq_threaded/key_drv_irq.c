...
ssize_t key_drv_read(struct file* filp, char __user* buf, size_t len, loff_t* off)
{
    ...
    if ((g_key == 0) && (filp->f_flags & O_NONBLOCK))
		return -EAGAIN;
...
{	
    ...
    for (int i = 0; i < 3; i++)
	{
		//ret = request_irq(gpio_keys_100ask[i].irq, gpio_key_isr, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING, "100ask_gpio_key", &gpio_keys_100ask[i]);
